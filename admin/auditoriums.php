...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['message'] = "Error deleting auditorium";
    }
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $auditorium_id = isset($_POST['auditorium_id']) ? (int)$_POST['auditorium_id'] : null;
    ...
    $seat_map = json_encode([
        'rows' => $rows,
        'cols' => $cols,
        'map' => generateSeatMap($rows, $cols) // Generate chessboard-style map
    ]);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['message'] = "Error saving auditorium: " . $conn->error;
    }
    $stmt->close();
    header('Location: auditoriums.php');
    exit;
...
<div class="admin-container">
    ...
    <div class="admin-content">
        ...
        <div id="auditorium-form" class="auditorium-form" style="display:none">
            <h2>Add New Auditorium</h2>
            <form method="POST" action="auditoriums.php" id="auditoriumForm">
                <input type="hidden" name="auditorium_id" value="">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="rows" name="rows" class="form-control" min="1" max="26" value="10" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="cols">Seats per Row</label>
                        <input type="number" id="cols" name="cols" class="form-control" min="1" max="50" value="15" required>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    <button type="button" class="btn btn-outline" onclick="document.getElementById('auditorium-form').style.display='none'">
                        Cancel
                    </button>
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php while ($auditorium = $auditoriums->fetch_assoc()): 
                    $seat_map = json_decode($auditorium['seat_map'], true);
                ?>
                <tr>
                    ...
                    <td class="actions">
                        ...
                        <a href="auditoriums.php?delete=<?= $auditorium['auditorium_id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this auditorium?')">
